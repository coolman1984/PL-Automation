# P&L A08 Automation — Detailed Agent Execution Plan V2 (Status Updated)

> Status audit date: 2026-08-27  
> Evidence source: files, code, cached test records, and diagnostics packaged in
> `P$L Project.zip`.  No item that depends on real Excel COM is considered
> complete until it passes on Windows against an approved disposable copy.

## 0. Current implementation status

### 0.1 Status legend

- ✅ **Implemented and statically verified** — code exists and Python compilation passed.
- ⚠️ **Implemented but not proven** — code exists, but real Windows Excel/NASCA evidence is missing or a remaining risk exists.
- ⬜ **Not completed** — the required deliverable or acceptance evidence does not exist.
- 🛠️ **Repaired in this review** — a confirmed defect was corrected in the packaged source.

### 0.2 Executive verdict

The project contains a strong fail-closed architecture and most planned modules,
but it is **not yet a production candidate**.  The source workbook is confirmed
to be a NASCA DRM file, so structural discovery, formula behavior, calculation,
save/reopen, and non-corruption can only be approved through desktop Excel COM
on the authorized Windows PC.

Four deterministic defects were repaired during this review:

1. 🛠️ Added the missing `working_fp` baseline that previously guaranteed an execution failure before save.
2. 🛠️ Corrected the September rule from hard-coded `A09` to the required `T09`.
3. 🛠️ Prevented the `★` character in the workbook name from crashing the Windows diagnostic console.
4. 🛠️ Corrected the idempotency integration test so it reruns the produced A08 workbook and requires explicit execution approval.
5. 🛠️ Wired Total PL discovery to the three business-sheet T08 columns instead of accepting a historical header match.
6. 🛠️ Added refusal of unsaved attached source workbooks and disabled events/auto-run code on isolated automation-owned Excel instances.
7. 🛠️ Restored the missing business-sheet updater import that would have stopped execution at the first update.
8. 🛠️ Added a dependency-free file/protection probe and deterministic smart routing before any Excel connection.
9. 🛠️ Updated the Windows launcher to locate and verify Python instead of assuming one fixed launcher or virtual-environment path.

These repairs pass Python compilation. The clean local test result is now **59
passed, 1 skipped** (the skipped test requires Windows Excel COM). They are not
marked as COM-proven.

### 0.3 Task-by-task audit

| Task | Status | Evidence and remaining work |
|---|---:|---|
| 1. Project scaffold | ✅ | Required folders, launcher, configuration, source modules, tests, and documentation exist. |
| 2. Constants, models, errors | ✅ | Implemented; static compilation passes. Current unit execution still needs a clean test run. |
| 3. Strict configuration | ✅ | Safe flags and A08 scope restrictions exist; tests are present. |
| 4. Excel session ownership | ⚠️ | Attach/open ownership, state restoration, unsaved-source refusal, and isolated pre-open event/macro controls exist. Must prove on Windows with NASCA. |
| 5. Workbook audit | ⚠️ | Fingerprints, hashes, links, pivots, connections, names, and VBA checks exist. Real workbook diagnostic evidence is incomplete. |
| 6. Safe file transaction | ⚠️ | Excel `SaveCopyAs`, collision-safe publication, and hash comparison exist. Full real `.xlsb` transaction is unproven. |
| 7. Header snapshots | ✅ | Bulk snapshot and merged-header helpers exist with unit tests. Real workbook geometry still needs capture. |
| 8. August block discovery | ⚠️ | Semantic locator, A08 idempotency, and Total PL lineage filtering are implemented. Unique blocks on all four real sheets are not proven. |
| 9. Dry-run workflow | ⚠️ | Workflow and report exist. Previous diagnostic opened Excel but crashed before inspection; rerun is required after the Unicode repair. |
| 10. Formula cloning/rewrite | ⚠️ | Native Excel copy and narrow quoted-version rewrite exist. Special formulas need real workbook evidence. |
| 11. Formatting/merge repair | ⚠️ | Column properties and August merge logic exist. Must prove no unrelated merge changes. |
| 12. VD Total update | ⚠️ | Updater exists; no saved/reopened real result is supplied. |
| 13. MX Total update | ⚠️ | Shared semantic updater exists; no real local-validation evidence is supplied. |
| 14. DA Total/combined update | ⚠️ | Ordered workflow exists; no combined Windows execution evidence is supplied. |
| 15. Total PL parser | ✅ | Conservative cross-sheet parser and rewrite tests exist. Formula varieties on the real sheet remain unknown. |
| 16. Real Total PL mapping audit | ⬜ | No complete row-classification report from the protected workbook exists. This blocks safe approval. |
| 17. Total PL A08 update | ⚠️ | Update code exists, but exact real-row lineage and reconciliation are unproven. |
| 18. Calculation/full validation | ⚠️ | Calculation wait and validation modules exist; the former guaranteed crash was repaired. Real calculation completion and zero mismatches remain unproven. |
| 19. Save/reopen/publication | ⚠️ | Transaction stages exist. No successful final workbook, reopen report, or repair-warning evidence is supplied. |
| 20. Reports/launcher/docs | ✅ | Launcher, README, run report, log, manifest, portable one-folder build, locked offline packages, self-check, and private repair path exist. Windows build execution remains required. |
| 21. Acceptance suite | ⚠️ | Local suite is complete: 59 passed, 1 Windows-only COM test skipped. Approved full COM acceptance report is still missing. |

### 0.4 Completion plan — execute in this order

#### Phase A — finish local code quality

- [x] Repair undefined working-copy fingerprint.
- [x] Correct September validation to `T09`.
- [x] Repair Unicode-safe diagnostic output.
- [x] Repair the idempotency integration test and execution approval gate.
- [ ] Add unit tests specifically proving `T09` remains immediately after the inserted `A08 | %` pair.
- [x] Wire Total PL candidate disambiguation to the proven business-sheet T08 columns, or fail with a precise blocker report.
- [x] Reject an attach-mode source workbook with unsaved changes.
- [x] Disable events and automatic macro execution on isolated automation-owned Excel instances; confirm this posture does not interfere with NASCA on Windows.
- [x] Run the entire unit suite in a clean Python environment and save the result. Result: 59 passed, 1 Windows-only COM test skipped.
- [x] Add a quick non-COM container/protection check and route NASCA to attach, normal XLSB to isolated Excel, and complex/unknown files to a safe stop or COM fallback.
- [x] Add portable one-folder Windows packaging, locked offline packages, integrity verification, self-check, and private non-global repair scripts.
- [x] Include locked Windows wheels for Python 3.12, 3.13, and 3.14 with a verified SHA-256 manifest.
- [ ] Generate the final executable on Windows 10/11 x64 and pass `SELF_CHECK.bat` there.

**Exit gate A:** Python compilation passes and the complete unit suite has zero failures.

#### Phase B — read-only proof on the authorized Windows PC

- [ ] Manually open the protected workbook in Excel and confirm NASCA access.
- [ ] Run the repaired diagnostic in attach mode and capture all sheet names, visibility, used ranges, names, links, connections, VBA status, and file format.
- [ ] Run dry-run in attach mode only; do not execute.
- [ ] Confirm unique August blocks on `VD Total`, `MX Total`, `DA Total`, and `Total PL`.
- [ ] Confirm the discovered September boundary is `T09` / `2026.009`.
- [ ] Export the Total PL row-classification and lineage audit without writing.
- [ ] Human-review every ambiguous, dynamic, external, or static Total PL row.

**Exit gate B:** dry-run says ready, A08 is absent, all four blocks are unique,
and every required Total PL row has an approved mapping.

#### Phase C — disposable-copy integration

- [ ] Record the original source SHA-256.
- [ ] Execute only through the approved working-copy transaction.
- [ ] Verify one `A08 | %` pair on each business sheet and `Total PL`.
- [ ] Verify `T08`, `S08`, and the entire September `T09` block remain unchanged except for the two-column shift.
- [ ] Compare representative detail, subtotal, percentage, blank, hidden, merged, and special-formula rows.
- [ ] Require completed Excel calculation with no formula errors.
- [ ] Require Total PL reconciliation within configured tolerance for all mapped rows.
- [ ] Save, close, reopen, and confirm Excel shows no repair/corruption warning.
- [ ] Confirm links, names, pivots, connections, VBA presence, sheet order, hidden state, and source hash are preserved.

**Exit gate C:** zero required validation failures, zero unresolved Total PL
mismatches, no repair warning, and unchanged source hash.

#### Phase D — idempotency and forced failures

- [ ] Rerun against the produced A08 workbook; it must stop before insertion.
- [ ] Test duplicate headers, missing September, unexpected Total PL columns, calculation timeout, save failure, and output collision.
- [ ] Confirm every failure publishes nothing and retains an actionable report.
- [ ] Confirm the user's existing Excel instance stays open and its settings are restored.

**Exit gate D:** all forced failures stop closed and the original workbook remains unchanged.

#### Phase E — production approval

- [ ] Package the clean source without caches, bytecode, old probe errors, work files, or failed outputs.
- [ ] Include the successful unit and COM acceptance reports.
- [ ] Let the finance owner compare the final workbook with the manual August process.
- [ ] Approve production only after written business sign-off.

### 0.5 Production decision

**Current decision: NO-GO.**  Continue from Phase A, then complete the Windows
proof gates.  The existing implementation must not be used on the only copy of
the source workbook.
