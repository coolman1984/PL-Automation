...
def test_catalog_distinguishes_available_from_planned_tools():
    ...
    assert by_name["write_range"]["status"] == "available"
    assert by_name["write_range"]["requires_backup"] is True
    assert by_name["set_formula"]["status"] == "available"
    assert by_name["publish_workbook"]["status"] == "planned"
...
def test_available_only_catalog_never_exposes_planned_tool_as_callable():
    ...
    assert "publish_workbook" not in {item["name"] for item in catalog["tools"]}
